...
from src.api import BaseOWSClient, LOTS, OWSAPIError

LOGGER = logging.getLogger(__name__)

DEFAULT_BATCH_SIZE = 500
DEFAULT_CONFIG_PATH = Path("config.yaml")
DEFAULT_CHECKPOINT_PATH = Path(".sisyphus/state/etl_lots_checkpoint.json")
...
def _as_record_list(value: object) -> list[Record]:
...
def _normalize_enstr(code: str) -> str:
    return code.strip()


def _collect_enstr_codes(value: object) -> list[str]:
    if value is None:
        return []

    if isinstance(value, str):
        normalized = _normalize_enstr(value)
    ...
    if isinstance(value, Mapping):
        ...
        for key in keys:
            ...
            if current is None:
                continue
            collected.extend(_collect_enstr_codes(current))

        for child_key in ("children", "items", "sub_items", "enstru_list"):
            children = item.get(child_key)
            if not isinstance(children, list):
                continue
            for child in cast(list[object], children):
                collected.extend(_collect_enstr_codes(child))

        deduped = list(dict.fromkeys(code for code in collected if code))
        return deduped

    if isinstance(value, list):
        collected_list: list[str] = []
        for list_item in cast(list[object], value):
            collected_list.extend(_collect_enstr_codes(list_item))
        return list(dict.fromkeys(code for code in collected_list if code))

    return []


def _extract_enstr_candidates(point: Record, record: Record) -> list[str]:
    candidates: list[str] = []

    for source in (point, record):
        direct_code = _first_non_empty_str(
            source,
            ("enstr_code", "enstru_code", "tru_code", "spec_enstr_code", "code"),
        )
        if direct_code:
            candidates.append(_normalize_enstr(direct_code))

    for source in (point, record):
        for list_key in ("enstru_list", "enstr_list", "tru_list"):
            nested_obj = source.get(list_key)
            candidates.extend(_collect_enstr_codes(nested_obj))

    deduped = list(dict.fromkeys(code for code in candidates if code))
    return deduped


def _extract_kato_candidates(point: Record, record: Record) -> list[str]:
    kato_codes: list[str] = []

    def _append_direct(source: Record) -> None:
        direct = _first_non_empty_str(source, ("kato_code", "kato", "katoCode"))
        if direct:
            kato_codes.append(direct)

    _append_direct(point)
    _append_direct(record)

    for source in (point, record):
        kato_list = _as_record_list(source.get("pln_point_kato_list"))
        for item in kato_list:
            code = _first_non_empty_str(item, ("kato_code", "kato", "code", "katoCode"))
            if code:
                kato_codes.append(code)

    return list(dict.fromkeys(code for code in kato_codes if code))


def _extract_points(record: Record) -> list[Record]:
    points = _as_record_list(record.get("point_list"))
    if points:
        return points
    return [record]
...
def _derive_row_id(
    ...
    *,
    ...
    if not has_nested and point_index == 0:
        return lot_id

    point_id = _to_uint64(
        point.get("id", point.get("point_id", point.get("pointId", point.get("pln_point_id")))),
        default=0,
    )

    if point_id > 0:
        return _stable_uint64(f"{lot_id}:{point_id}:{enstr_code}:{kato_code}")
    ...
    signature = "|".join(
        [
            ...
            enstr_code,
            kato_code,
            _first_non_empty_str(point, ("mkei_code", "mkei", "unit_code", "unitCode")),
            str(point.get("amount", point.get("sum"))),
...
def _fetch_existing_latest_by_ids(
    ...
    query = (
        ...
        "SELECT id, "
        f"{argmax_columns} "
        "FROM lots WHERE id IN %(ids)s GROUP BY id"
        ")"
    )
...
def _upsert_batch(
    clickhouse: ClickHouseClient, rows: Sequence[ParsedLotRow]
) -> tuple[int, int, int]:
    if not rows:
        return 0, 0, 0

    latest_by_id: dict[int, ParsedLotRow] = {}
    for row in rows:
        row_id = row[0]
...
def _filter_rows_by_announcement_scope(
    ...
    for row in rows:
        ...
        customer_bin = row[3] if row[3] > 0 else ann_customer_bin
        if customer_bin not in target_bins:
            stats.skipped_non_target_bin += 1
            continue

        if customer_bin != row[3]:
            filtered.append((row[0], row[1], row[2], customer_bin, *row[4:]))
            continue
...
def _parse_lot_record(
    record: Record,
    target_bins: set[int],
    stats: DataQualityStats,
    fallback_customer_bin: int,
) -> tuple[list[ParsedLotRow], datetime | None]:
    stats.raw_records += 1

    lot_id = _to_uint64(record.get("id", record.get("lot_id", record.get("lotId"))), default=0)
    if lot_id <= 0:
    ...
    created_at = _parse_datetime(
        record.get("created_at", record.get("crdate", record.get("publish_date"))),
        default=now,
    )
    ...
    lot_number = _first_non_empty_str(
        record,
        ("lot_number", "lotNumber", "number_lot", "lot_num", "number"),
    )
    ...
    has_point_list = isinstance(record.get("point_list"), list)
    points = _extract_points(record)
    rows: list[ParsedLotRow] = []

    for point_index, point in enumerate(points):
        enstr_codes = _extract_enstr_candidates(point, record)
        if not enstr_codes:
            stats.skipped_missing_enstr += 1
            LOGGER.warning("Lot %s skipped due to missing ENSTR code", lot_id)
            continue

        kato_codes = _extract_kato_candidates(point, record)
        kato_candidates = kato_codes if kato_codes else [""]
        ...
        quantity = _to_decimal(
            point.get("count", point.get("quantity", point.get("qty", record.get("count")))),
            scale=3,
            default=Decimal("0.000"),
        )
        amount = _to_decimal(
            point.get("amount", point.get("sum", record.get("amount"))),
            scale=2,
            default=Decimal("0.00"),
        )
        unit_price = _to_decimal(
            point.get("unit_price", point.get("price", record.get("unit_price"))),
            scale=2,
            default=Decimal("0.00"),
        )
        ...
        if (
            ...
            and quantity > Decimal("0.000")
            and amount > Decimal("0.00")
        ):
            unit_price = (amount / quantity).quantize(Decimal("1.00"), rounding=ROUND_HALF_UP)

        for enstr_code in enstr_codes:
            for kato_code in kato_candidates:
                row_id = _derive_row_id(
                    lot_id,
                    point,
                    point_index,
                    has_nested=has_point_list
                    or isinstance(record.get("enstru_list"), list)
                    or isinstance(record.get("pln_point_kato_list"), list),
...
def load_lots(
    ...
    *,
    ...
    if force:
        LOGGER.info("Force mode enabled: loading all lot records for configured BINs")
    ...
    try:
        for endpoint_index, _ in enumerate(endpoints):
            ...
            for bin_index, bin_value in enumerate(bins_ordered):
                ...
                endpoint_with_bin = f"{LOTS}/bin/{bin_value}"
                ...
                for item in _iterate_bin_endpoint(ows_client, endpoint_with_bin, bin_value, since):
                    rows, modified_at = _parse_lot_record(item, target_bins, stats, bin_value)
                    ...
                    while len(batch_rows) >= batch_size:
                        ...
                        _write_checkpoint(checkpoint_path, checkpoint)
                        LOGGER.info(
                            "Lots progress: parsed=%d inserted=%d duplicates=%d unchanged=%d",
        ...
        LOGGER.error("Lots ETL failed; checkpoint persisted for recovery: %s", error)
        raise

    quality = stats.as_dict()
    LOGGER.info("Lots ETL finished with quality stats: %s", quality)
    return quality
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ETL pipeline for OWS lots endpoint")
    ...
    LOGGER.info("Lots ETL summary: %s", summary)
